...
| A static import-cycle check over `src/features/editor/` | SHIPPED 2026-09-04 — and it is repo-wide rather than editor-scoped. `scripts/sourceLint.test.ts`'s "has no runtime import cycles" walks every runtime edge under `src/` (`import type` and dynamic `import()` excluded, since neither can participate in an initialisation cycle) and fails on a cycle in any direction. It found **one live cycle on its first run** — `export/index -> exportNote -> html -> editor/index -> RichEditor -> export/index` — latent for exactly the reason N's was: it runs through `editor/index`, which pulls `markdown.ts`, which builds from `editorExtensions` at module top level, and it had not fired only because the evaluation order happened to work. Broken in `html.ts` by importing the four editor leaves instead of the barrel, which also reclaimed **1,222 B** (349,773 -> 348,551) because the barrel was dragging `RichEditor` into the export path; breaking the same cycle at the other end removed it equally well and COST 329 B, so which end you pick is worth measuring. `importCycle.test.ts` is kept alongside: the graph check reads imports, that one evaluates them in the fatal order. |
...
| `e2e/imageResize.spec.ts`'s drag-resize persistence test | **SOLVED 2026-09-07. It was never a product defect, and the four earlier characterisations here were all wrong.** Root cause: the test reloaded inside the 300ms `AUTOSAVE_DELAY_MS` window. `useFlushTriggers` listens to `beforeunload`, which can only START an asynchronous write and cannot wait for one, so the reload raced the debounce and the note was often never written at all. Two symptoms, one race, which is why it read as two different bugs: when a partial save had landed the width reverted to its pre-drag value (the recorded ~172px), and when none had, the note's own TITLE never persisted, so the row the test clicks after reloading did not exist and it timed out on `getByRole('button', { name: /Resizable/ })` — a failure that never even reached the width assertion. Nothing was load-dependent; every run below was on an idle machine. Measured: with no wait, 1 pass in 3; with a 500ms wait, 6 of 6. Fixed by polling IndexedDB for the ENCODED WIDTH (`![alt\|388](...)`) before reloading, not by a fixed timeout and not by the note row appearing — every keystroke restarts the debounce, so a row with the right title only proves some EARLIER save landed, not the one carrying the drag. The pattern was already in this repo twice and simply unused here: `noteListHeader.spec.ts`'s `waitForSetting` and `smoke.spec.ts`'s direct IndexedDB read, whose docblock already named the mechanism — "A human cannot click and reload inside that gap; Playwright does it every time." Verified 6/6 green, and fault-injected (the `onUp` dispatch removed) to confirm the test still fails when the drag genuinely does not reach the document. |
