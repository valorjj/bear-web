...
| **Q typography settings** | IN PROGRESS on branch `q-typography-settings`. Spec: `docs/superpowers/specs/2026-09-03-q-typography-settings-design.md`. Plan: `docs/superpowers/plans/2026-09-03-q-typography-settings.md`. Five controls — font size, line height, line width, paragraph spacing, paragraph indent — in a modal opened from the sidebar footer, applied live, persisted per device, carried into every export. From the user's report: "the content area looks cramp." Two decisions were revised during specing and both are recorded in the spec: the hook is shaped like `useTheme`, NOT `useSetting`, because typography needs a paint-time mirror and `useSetting` renders at its fallback until IndexedDB answers (invisible for a sort order, a full-note reflow for a font size); and Q closes the two editor/export divergences rather than widening them — measured, the export's `h1/h2/h3` are `1.6/1.35/1.15em`, the PRE-M9a scale, against the editor's derived `1.728/1.44/1.2`, so they are not merely literals but the wrong ones. |
| The bundle ceiling is **351,000 B**, raised by the user on 2026-09-03 | Measured at `349c9f6`: the eager closure is **347,855 B**, so the frozen 348,000 ceiling had 145 B left and sub-project Q could not fit under it. The raise was granted only after the freeze's other exits were MEASURED rather than argued, and two of those measurements correct what this row used to claim. **A fourth `React.lazy` root makes the bundle WORSE**: converting `ThemeDialog` (a static import gated on `open`) to `React.lazy` took the closure from 347,854 B to 348,176 B — 4 eager chunks became 6, and the re-hoisted shared runtime cost 322 B more than deferring the whole dialog saved. **And `themes-*` is not the theme code.** Rolldown's chunk names are arbitrary: in that same spike build the ~234 KB chunk came back named `notes-*` while a NEW 1,049 B chunk took the name `themes-*`. The 1,049 B is the theme roster plus its dialog; the 234 KB is Tiptap, ProseMirror, React and lowlight wearing the name. Splitting "themes" out therefore nets about **-322 B** — a regression, not a sub-project. This row previously named it "the obvious candidate"; it was reading a filename. The raise is conditional: if Q's finished closure lands well under 351,000, the ceiling comes DOWN to the measured figure plus ~3 KB. Reasoning in `scripts/bundleSize.test.ts`'s docblock. |
