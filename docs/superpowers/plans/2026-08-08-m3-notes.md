...
export function useAutosave({ initial, save, discard, delayMs = AUTOSAVE_DELAY_MS }: AutosaveOptions): Autosave {
  ...
  const flush = useCallback(() => {
    ...
    void saveRef.current(pending).then(
      () => setFailed(false),
      () => {
        // Guarded: two saves can overlap, and each rejection handler closes
        // over its own `previous`. Without this check, an older save that
        // fails *after* a newer one succeeded would stomp the newer, correct
        // marker back to a stale value and report a failure that did not
        // happen — which Task 7 renders as a user-visible message.
        if (savedRef.current === pending) savedRef.current = previous;
        setFailed(true);
      },
    );
  }, [cancelTimer]);
...
> renders as a user-visible message. The `if (savedRef.current === pending)`
> guard is the fix, and the test list gains a case for overlapping in-flight
> saves. Also amended: the `visibilityState` cleanup uses
