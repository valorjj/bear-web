...
describe('useAutosave', () => {
  ...
  it('flushes on visibilitychange when the page becomes hidden', async () => {
    ...
    // `delete (document as Partial<Document>).visibilityState` fails tsc with
    // TS2704 — `Partial` preserves `readonly`. Same [[Delete]] operation.
...
export function useAutosave({ initial, save, discard, delayMs = AUTOSAVE_DELAY_MS }: AutosaveOptions): Autosave {
  ...
  const flush = useCallback(() => {
    ...
    // Two saves can overlap — type, pause past the debounce, keep typing — and
    // each handler closes over its own `previous`. Only the latest claim may
    // mutate shared state. Comparing text instead of claiming a token is NOT
    // sound: after a rollback, `savedRef.current` can coincidentally equal an
    // older save's own `pending`, so a value guard passes for a superseded
    // write.
    const token = ++saveSeqRef.current;

    void saveRef.current(pending).then(
      () => {
        if (saveSeqRef.current !== token) return; // superseded
        setFailed(false);
      },
      () => {
        if (saveSeqRef.current !== token) return; // superseded
...
> **Amended 2026-08-08, after the Task 6 review.** The rejection handler
> originally read `savedRef.current = previous;` unguarded. Two saves can
> overlap — type, pause past the debounce, keep typing — and each handler
> closes over its own `previous`. An older save failing after a newer one
> succeeded would stomp the correct marker and set `failed`, which Task 7
> renders as a user-visible message. A monotonic `saveSeqRef` token claimed per
> flush is the fix — a value guard is not sound, because after a rollback
> `savedRef.current` can coincidentally equal an older save's own `pending`.
> The test list gains both overlap orderings. Also amended: the
> `visibilityState` cleanup uses
> `Reflect.deleteProperty`, because the original `delete (document as
> Partial<Document>).visibilityState` fails `tsc` with TS2704.
