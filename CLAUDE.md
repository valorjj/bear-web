...
2815 unit tests pass and 112 skip (the server integration tests, which skip
...
**The rulings live in `docs/rulings/`, not here.** 399 bullets across 13 files,
...
| any `aria-*` attribute or accessible-name assertion; `Icon.tsx`, `SidebarRow.tsx`'s explicit space, `NoteListItem.tsx`'s label, `preview.ts`'s `snippetLines`, `Button.tsx`'s variants and its `ariaHasPopup`/`ariaExpanded`, `NoteList.tsx`'s header, `ScopeMenu.tsx`'s roles, `ConfirmDialog`'s Cancel, `EditorContextMenu.tsx`'s roles and `Shift+F10` route, `HighlightPalette.tsx`, `CommandPalette.tsx`'s combobox/listbox pair and its `data-palette-header` rows | [accessibility.md](docs/rulings/accessibility.md)                                                                                      |
