...
| U heading links: `[[Note/Heading]]`                                | complete |
...
**Four capabilities need the network, and everything else still works
offline exactly as before:** sync (since D2), PDF export (since G),
publishing a note to a public URL (since M), and IMPORTING one (since W).
PDF export and publishing each need an account; both `ExportMenu` items mark
themselves `aria-disabled` when signed out and name the reason in their
accessible name.

**Import deliberately needs none**, and that asymmetry is the feature rather
than an oversight: the recipient of a shared link is usually someone who has
never used this app, so requiring a sign-in to accept a note would make it
useless in its commonest case. It writes straight into IndexedDB, works in
guest mode, and rides up on the next push if the recipient ever signs in.
...
- **`playwright.config.ts` hardcodes port 4173 with `reuseExistingServer`, and
  ...
  **Third direction, met while reproducing a load-sensitive failure:** two
  `playwright test` processes share the port and the FIRST to finish tears the
  server down, so the other dies mid-run with `ERR_CONNECTION_REFUSED` — which
  reads as a fault in the thing you are chasing. To co-run load deliberately,
  start the server yourself (`npm run build && npm run preview &`) so neither
  run owns it, then run real spec files alongside.
...
- **Several e2e tests fail under machine load, and the failures look like
  ...
  **But "passes alone, fails in the suite" is not itself evidence of
  contention, and reading it that way cost most of a day.**
  `footnotes.spec.ts`'s toolbar test matched every pattern above — clean
  alone, red in about half of all back-to-back full runs — and was a genuine
  focus bug that duplicated document content (see the Tiptap `focus()` bullet
  under Toolchain surprises). A failure reproducible at ~50% has a cause worth
  finding. Two things settle these, and both were learned the hard way here:
  the base-vs-branch comparison needs **three runs a side**, because two clean
  base runs gave a false all-clear and nearly pinned a real bug on an
  unrelated diff; and **an instrument that slows the run hides the race** —
  `--trace` took the suite from 38s to 60s and it stopped reproducing at all,
  as did adding intermediate probes. Dump state only ON FAILURE, at the
  original timing.
...
- **Tiptap's `focus()` is deferred to a `requestAnimationFrame`, so a chrome
  button keeps the KEYBOARD for a frame after its command runs.** Chromium
  focuses a `<button>` synchronously on mousedown; `@tiptap/core`'s
  `commands/focus.ts` does not focus the view until the next frame. A `Space`
  or `Enter` pressed in that window ACTIVATES THE BUTTON AGAIN — and for an
  insert command it inserts again. One frame normally beats anyone's reaction
  time, which is how this survived from M4 to 2026-09-17; under load it is
  wide open, and clicking Bold then immediately pressing space re-toggles bold
  on any machine. Every chrome control that runs an editor command therefore
  carries `onMouseDown={keepEditorFocus}`
  (`src/features/editor/keepEditorFocus.ts`). It is NOT the same concern as
  `pinAllSelectionStep`: the editor's SELECTION already survived a click on
  chrome, and DOM focus did not. Menus are exempt only because their buttons
  unmount on choose.

- **jsdom implements no `ResizeObserver`.** `new ResizeObserver` throws and
  React reports only "An error occurred in the <X> component" with nothing
  naming the missing API — 182 tests failed on one constructor.
  `vitest.setup.ts` stubs it as a NO-OP, never a fake that fires: jsdom has no
  layout engine, so every box it could report would be zero, and a stub
  delivering zeroes would let a unit test assert a geometry decision the real
  browser makes differently. Geometry belongs in Playwright.

- **An emulated mobile page reports `innerHeight` as 2121 at
  `document_start`.** A Playwright `addInitScript` that seeds a size from it
  bakes in a screen two and a half times too tall for the rest of the test.
  `e2e/fixtures/fakeViewport.ts` did exactly that, so `visibleBottom()`
  returned 2121 for every phone test that never set a keyboard inset and every
  "does this fit below the anchor" decision answered yes. Derive such a value
  on READ; never seed it at init time. Tests that set an explicit inset
  overwrote the bad seed, which is why it went unnoticed for so long.
...
- **The bundle guard used to measure "the largest single JS asset", and that
  ...
  and Rolldown responded by re-chunking the eager code itself across THREE
  files (`themes-*`, `index-*`, `EmptyState-*`) instead of one. The old guard
  kept looking at whichever file was largest — `themes-*` at 230,427 B — and
  PASSED, while the real eager payload sitting behind the shortcut was already
  343,043 B, over the ceiling in force at the time. `scripts/bundleSize.test.ts`
  now reads `dist/.vite/manifest.json` and sums the entry chunk's transitive
  STATIC `imports` closure, never following `dynamicImports` (the genuinely
  lazy code the ceiling exists to exclude). **`build.manifest: true` in
  `vite.config.ts` must stay enabled** — turning it off fails the test loudly
...
- **Every floating surface composes `src/ui/menuStyles.ts`** — menus, the
  toolbar overflow sheet, anchored panels — rather than spelling
  `shadow-popover` and its radius, border and row styling itself. Enforced by
  `scripts/sourceLint.test.ts`, with a named allowlist for the surfaces that
...
**The rulings live in `docs/rulings/`, not here.** 600 bullets across 13 files,
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `src/data/tags/` — `parseTags`, `findTagRanges`, `normalizeTag`, `MASK`, the fence and mask helpers; `TAG_INDEX_VERSION`; any prose introducing the mask character's escape sequence; `TagAutocomplete.ts`'s `tagAutocompleteMatchAt` / `matchingTags`, `tagAutocomplete.test.ts`                                                                                                                                                                                                                                                                                                                              | [tag-grammar.md](docs/rulings/tag-grammar.md)                                                                                          |
...
| `NoteEditor.tsx`, `useAutosave.ts` (`defer`, `maxDeferMs`, `AUTOSAVE_MAX_DEFER_MS`), `useNotes.ts`, `derive.ts`, `useFlushTriggers.ts`; `AppShell`'s `key={…}` / `seed`; any `useLiveQuery` whose deps are not `[]`, and any write GATED on one (`useTagTree.reveal`); `notes.purge` / `notes.save` call sites; `src/features/graph/useGraphSnapshot.ts`                                                                                                                                                                                                                                                       | [notes-lifecycle.md](docs/rulings/notes-lifecycle.md)                                                                                  |
...
| `TagPill.ts` (`tagDecorations`, `tagSyntaxDecorations`, `tagRangeAt`, the `mousedown` handler), `LinkPill.ts`'s `linkSyntaxDecorations`, `blockText.ts` (`maskedBlockText`), `RichEditor`'s `activateRef`, `AppShell.handleActivateTag`, `--bear-tag-fill*`, `--bear-tag-icon`, any selector counting `.bear-tag` / `.bear-link`, `tagAgreement.test.ts`, `TagAutocomplete.ts`, `tagAutocomplete.test.ts`                                                                                                                                                                                                      | [tag-pills.md](docs/rulings/tag-pills.md)                                                                                              |
| `src/features/export/` — `html.ts` (`decoratePills`, `inlineImages`'s `data-src`, `sourcePayload`, `EXPORT_TOKEN_NAMES`), `exportNote.ts`, `requestPdf.ts`, `filename.ts`, `ExportMenu.tsx`; `NoteEditor.handleExport`; `server/src/routes/export.ts`, `server/src/routes/share.ts`, `server/src/routes/publicPage.ts`'s banner, `server/pdf/`; `src/features/import/` (`parseSharedPage.ts`, `ImportGate`, `ImportSheet.tsx`), `src/data/importNote.ts`, `src/features/publish/staleBuild.ts`; the `export.*`, `publish.*` and `import.*` i18n keys and `ALLOWED_IDENTICAL`                                   | [export.md](docs/rulings/export.md)                                                                                                    |
| `src/styles/*.css`, `themes.ts`, `app/theme.ts`, `index.html`'s inline script, `Pane.tsx`, `Resizer.tsx`, `Button.tsx`, `ThemePicker.tsx`, `RichEditor.tsx`, `SidebarDrawer.tsx`, `NoteEditor.tsx`'s outer class list; a new `--bear-*` property, `[data-theme]` block, spacing / radius / shadow / `outline-none` utility; any `bg-sidebar` / `bear-sidebar-scope` / `bear-app-palette` class, or `h-full` on a pane child; `src/ui/menuStyles.ts`, `BottomToolbar.tsx`'s `overflow` flags, `ToolbarOverflowSheet.tsx`, `RichEditor.tsx`'s `data-toolbar-space`                                               | [design-tokens-and-layout.md](docs/rulings/design-tokens-and-layout.md)                                                                |
...
| any `aria-*` attribute or accessible-name assertion; `keepEditorFocus.ts` or an `onMouseDown` on editor chrome; `Icon.tsx`, `SidebarRow.tsx`'s explicit space, `NoteListItem.tsx`'s label, `preview.ts`'s `snippetLines`, `Button.tsx`'s variants and its `ariaHasPopup`/`ariaExpanded`, `NoteList.tsx`'s header, `ScopeMenu.tsx`'s roles, `ConfirmDialog`'s Cancel, `EditorContextMenu.tsx`'s roles and `Shift+F10` route, `HighlightPalette.tsx`, `CommandPalette.tsx`'s combobox/listbox pair and its `data-palette-header` rows, `TagAutocomplete.ts`'s `view()` role mirror and `tagAutocomplete.test.ts` | [accessibility.md](docs/rulings/accessibility.md)                                                                                      |
| `e2e/appearance.spec.ts`, `smoke.spec.ts`, `contrast.spec.ts`, `scripts/*.test.ts`, the tsconfig `include`/`types`; a `lucide-react` import outside `Icon.tsx`; a `[role="…"]` selector or `.closest()` inside `page.evaluate`; **any test you are about to edit because a restyle made it fail**; `e2e/tags.spec.ts`'s autocomplete tests, `tagAutocomplete.test.ts`                                                                                                                                                                                                                                          | [testing-and-tooling.md](docs/rulings/testing-and-tooling.md)                                                                          |
